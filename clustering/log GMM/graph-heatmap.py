...
from DistanceCS import Distence
import warnings
...
data = pd.read_excel('supercleanGMMFiltered.xlsx' )
data = data.fillna('0')
data = data.dropna()
ddata = pd.read_excel('supercleanGMMFiltered-distances.xlsx' ).fillna(0).to_numpy(float)
flattenddata = list(chain.from_iterable(ddata)) ###
#delta = max(flattenddata)-min(flattenddata) ###
sequenceNames = list(pd.read_excel('supercleanGMMFiltered.xlsx')['Sequence'])
index = 15
mini = 400#np.min(np.array(datas, dtype="float")) 
maxi = 1300#np.max(np.array(datas, dtype="float"))
...
random_states = [42, 67, 96] # type int, default=None
n_inits = [30, 50, 70] # type int, default=10
gammas = [1.0] # type float, default=1.0
affinitys =  ['precomputed']# str or callable, default=’rbf’
n_neighbors = [10,30,50,70] # int, default=10
     ...
     # Calculate histograms for each cluster based on class labels from df1
        ...
        for cluster_label in set(data['labels']):
                ...
                for idx in range(len(cluster_data)):
                        ...
                        try:colers[colerize(cluster_data['Peak1 [nm]'  ][idx] ,cluster_data['Peak1 [nm]'  ],index)] = [*colers[colerize(cluster_data['Peak1 [nm]'  ][idx],cluster_data['Peak1 [nm]'  ],index )],float(cluster_data['Peak 1 a-WAV'  ][idx])]
                        except:pass
                        try:colers[colerize(cluster_data['Peak2 [nm]'  ][idx] ,cluster_data['Peak2 [nm]'  ],index)] = [*colers[colerize(cluster_data['Peak2 [nm]'  ][idx],cluster_data['Peak2 [nm]'  ],index )],float(cluster_data['Peak 2 a-WAV'  ][idx])]
                        except:pass
                        try:colers[colerize(cluster_data['Peak3 [nm]'  ][idx] ,cluster_data['Peak3 [nm]'  ],index)] = [*colers[colerize(cluster_data['Peak3 [nm]'  ][idx],cluster_data['Peak3 [nm]'  ],index )],float(cluster_data['Peak 3 a-WAV'  ][idx])]
                        except:pass
                ...
                plt.close()
                cldata.append(np.mean(np.array(cluster_data[['Peak 1 a','Peak 1 b','Peak 1 c','Peak 2 a','Peak 2 b','Peak 2 c','Peak 3 a','Peak 3 b','Peak 3 c']].to_numpy(),dtype=float),axis=0))
        dis = Distence(pd.DataFrame(cldata,columns=['Peak 1 a','Peak 1 b','Peak 1 c','Peak 2 a','Peak 2 b','Peak 2 c','Peak 3 a','Peak 3 b','Peak 3 c']))
