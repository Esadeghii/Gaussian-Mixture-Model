...
import pandas as pd
def Distence(df):
    # df = pd.read_excel('./xlsx files/diff-GMM-test.xlsx')
    ...
    for i in range(len(df)):
        sum1 = sum((df.loc[i]['Peak 1 a'], df.loc[i]['Peak 2 a'], df.loc[i]['Peak 3 a']))
        if sum1 == 0.0:
            continue
        q = [{'weight': df.loc[i]['Peak 1 a'] / sum1, 'mu': df.loc[i]['Peak 1 b'], 'var_inv': (df.loc[i]['Peak 1 c']) ** 2},
            {'weight': df.loc[i]['Peak 2 a'] / sum1, 'mu': df.loc[i]['Peak 2 b'], 'var_inv': (df.loc[i]['Peak 2 c']) ** 2},
            {'weight': df.loc[i]['Peak 3 a'] / sum1, 'mu': df.loc[i]['Peak 3 b'], 'var_inv': (df.loc[i]['Peak 3 c']) ** 2}]
        # q = [d for d in q if any(d.values()) != 0.0]
        q = [d for d in q if not all(value == 0.0 or np.isnan(value) for value in d.values())]

        for j in range(len(df)):
            if i != j :
                sum2 =sum((df.loc[j]['Peak 1 a'], df.loc[j]['Peak 2 a'], df.loc[j]['Peak 3 a']))
                if sum2 == 0.0:
                    continue
                p = [{'weight': df.loc[j]['Peak 1 a']/sum2, 'mu':df.loc[j]['Peak 1 b'], 'var_inv':(df.loc[j]['Peak 1 c'])**2},
                    {'weight':df.loc[j]['Peak 2 a']/sum2, 'mu':df.loc[j]['Peak 2 b'], 'var_inv':(df.loc[j]['Peak 2 c'])**2},
                    {'weight':df.loc[j]['Peak 3 a']/sum2, 'mu':df.loc[j]['Peak 3 b'], 'var_inv':(df.loc[j]['Peak 3 c'])**2}]
    ...
    return df#.to_excel('./xlsx files/diff-GMM-test-Distance-CS.xlsx', index=False)
