            ...
            if epoch_index == 0 or epoch_index % scale == (scale-1):
                ...
                for i in range(clusters_distance_latent_train.shape[0]):
                    for j in range(i+1, clusters_distance_latent_train.shape[0]):  # To avoid calculating distances twice (i to j and j to i)
                        ...
                        x1_tensor = torch.from_numpy(clusters_distance_latent_train[i]).float()  # Convert to float type if necessary
                        x2_tensor = torch.from_numpy(clusters_distance_latent_train[j]).float()
                        distance_train_latent = 1 - torch.nn.functional.cosine_similarity(x1_tensor, x2_tensor, dim=0)
                ...
                for i in range(clusters_distance_latent_valid.shape[0]):
                    for j in range(i+1, clusters_distance_latent_valid.shape[0]):  # To avoid calculating distances twice (i to j and j to i)
                        #distance_val_latent = np.linalg.norm(clusters_distance_latent_valid[i] - clusters_distance_latent_valid[j])
                        x3_tensor = torch.from_numpy(clusters_distance_latent_valid[i]).float()  # Convert to float type if necessary
                        x4_tensor = torch.from_numpy(clusters_distance_latent_valid[j]).float()
                        distance_val_latent = 1 - torch.nn.functional.cosine_similarity(x3_tensor, x4_tensor, dim=0)
                        #distance = torch.linalg.norm(latent_code_mean[i] - latent_code_mean[j])
