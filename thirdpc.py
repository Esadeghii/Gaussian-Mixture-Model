...
def generate_vertical_circle(center, radius, color, opacity):
    """Generates a vertical circle in 3D with a given center, radius, color, and opacity."""
...
def pca_visualize_3D_centroids_and_std(centroids, std_devs, explained_variance_ratio, circle_opacity=0.1):
    """Visualizes the centroids and their standard deviations as vertical circles in 3D."""
    clusterscolor = {
        'R-FR': 'purple',
        'G': '#90EE90',
        'N': 'black',
    ...
    for i, row in centroids.iterrows():
        ...
        fig.add_trace(go.Scatter3d(
            ...
            z=[row['PC3']],
            mode='markers',
            marker=dict(size=5, color=color, opacity=1),
            name=f"{label} (Var: x={explained_variance_ratio[0]:.2f}, y={explained_variance_ratio[1]:.2f}, z={explained_variance_ratio[2]:.2f})"
        ))

    # Add vertical circles representing third highest standard deviations
    for i, row in centroids.iterrows():
        center = row[['PC1', 'PC2', 'PC3']].values
        std_devs_row = std_devs.loc[i, ['PC1', 'PC2', 'PC3']].values
        radius = np.partition(std_devs_row, -3)[-3]  # Third highest std dev
        color = clusterscolor[row['Labels']]
        fig.add_trace(generate_vertical_circle(center, radius, color, circle_opacity))
