...
def pca_visualize_3D(principalComponents, principalDf, wavelength, z):
    ...
    for i in range(len(wavelength)):
        ...
        if wavelength[i] <= 590:
            ...
            wl_color.append('> 800 nm(Near IR)')
    principalDf['Wavelength'] = wavelength
    fig = px.scatter_3d(
        principalDf,
        x = 'PC1',
        y = 'PC2',
    ...
    add_wavelength_annotations_for_3d(fig, wavelength, z[:,0])
...
def conduct_visualizations(path_to_dataset: str, path_to_model, pca_tuple: tuple):
    ...
    if pca_tuple[2] is True:
        pca_visualize_3D(principalComponents, principalDf, wavelength, z)
...
'all-results/1-20-22-models-info/a19lds19b0.007g1.0d1.0h13.pt', (True, True, True))
conduct_visualizations('data-for-sampling/processed-data-files/16-base_norm-sliding-window-result.npz', 
'all-results/1-20-22-models-info/a19lds19b0.007g1.0d1.0h13.pt', (True, True, True))
